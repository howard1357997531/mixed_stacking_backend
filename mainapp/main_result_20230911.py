...
# Global Constants
V = (220,220,240)
INPUT_PATH = 'box_volume_11items_40.csv'
...
CONFIG = {
    ...
    "Ind": 300, # num_individuals
    "Eli": 40, # num_elites
    "Mut": 40, # num_mutants
    "EliCPro": 0.8, # eliteCProb
    "pat": 16, # patient
...
def distribute_randomly(total, num_items):
    """Distribute the total into num_items parts randomly."""
    ...
    for i in range(decoder.num_opend_bins):
        for j, box in enumerate(decoder.Bins[i].load_items):
            ...
            box_data.append([
                ...
                int(np.ceil(position[0])), int(np.ceil(position[1])), int(np.ceil(position[2])),
...
def match_box_data(box_data, baseline_boxes):
    ...
    for index, row in box_data.iterrows():
        ...
        matched_box_name, orientation = match_box_name(int(row['width']), int(row['height']), int(row['depth']), baseline_boxes)
...
def visualize_boxes(box_data, container_data, result_folder, csv_base_name):
    ...
    for index, row in box_data.iterrows():
        ...
        # Change color of the last drawn box to blue while keeping edges black
...
def model_training(inputs, CONFIG):
    ...
    model = BRKGA(
        ...
        num_individuals=CONFIG["Ind"], 
        num_elites=CONFIG["Eli"], 
        num_mutants=CONFIG["Mut"], 
        eliteCProb=CONFIG["EliCPro"], 
        multiProcess=CONFIG["multi"])
    model.fit(patient=CONFIG["pat"], verbose="ver")
...
def activate_cal(worklist_id):
    ...
    inputs = load_and_process_data(input_path)
    ...
    box_data = match_box_data(box_data, baseline_boxes)
    visualize_boxes(box_data, container_data, result_folder, csv_base_name)
