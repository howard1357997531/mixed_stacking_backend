    ...
    def update(self, box, selected_EMS, min_vol = 1, min_dim = 1, verbose=False):
        ...
        for EMS in self.EMSs.copy():
            if self.overlapped(ems, EMS):
                ...
                new_EMSs = [
                [np.array((x1, y1, z1)), np.array((x3, y2, z2))], # left of box
                [np.array((x4, y1, z1)), np.array((x2, y2, z2))], # right of box
                [np.array((x1, y1, z1)), np.array((x2, y3, z2))], #front of box
                [np.array((x1, y4, z1)), np.array((x2, y2, z2))], # back of box
        ...
        for EMS in self.Bins[k].EMSs:
            ...
            for direction in [1,3]:
                ...
                if self.fitin((d, w, h), EMS):
                    x, y, z = EMS[0]
                    distance = np.power(D-x-d, 2) + np.power(W-y-w, 2) + np.power(H-z-h, 2)
