...
def getdata(orderId):
    ...
    box_positions_final_path = os.path.join(settings.MEDIA_ROOT, f'ai_figure/Figures_{orderId}', 'box_positions_final.csv')
    # -----------------------
            ...
            # if self.Robot_sensor3 and not self.removelock and not self.Robot_sensor1:
                ...
                if result[0][0] != '#0'or result[1] != '-1':                
                    while self.Pc_system:
                        ...
                        if len(Box_ID)!=0 and not len(self.name_list) == 0:
