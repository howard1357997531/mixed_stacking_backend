...
class Dimension_3D():
    ...
    def depth_filter(self, point_cloud):
        """
            founding min depth and max depth by depth threshold
            Input: 
                point_cloud: array - > point cloud for current process

            Output:
                list  ->  min depth and max depth 
        """    
    ...
    def find_closest_corner(self, given_point, given_mask):
        """
            founding closest corner for given point by given mask
            Input: 
                given_point: array - > points for the bounding box rectangle.
                given_mask: array -> points of mask

            Output:
                array -> the closest point for given mask.
    ...
    def sort_rectangle(self,rectangle):
        """
            Sorting given array, the output sould be 
                        0        2
                        1        3           

            where number means index for output array.   
            Input: 
                rectangle: array - > rectangle array, shape sould be (4,2).

            Output:
               array -> sorted rectangle array.
    ...
    def mask_processing(self,point_cloud,min_depth, max_depth):
        """
            Using depth threshold  to find the desired plane and morphologyEx to smooth the plane.
            Input: 
                point_cloud: array - > point cloud.
                min_depth : float -> min depth for current region.
                max_depth : float -> max depth for current region.
            Output:
                mask : array -> current founding mask by depth threhold and morphologyEx.
                points: array -> points for the mask.
        ...
        mask = cv2.morphologyEx(mask_.astype(np.uint8), cv2.MORPH_OPEN, np.ones((13, 13), np.uint8))
    ...
    def find_rectangle(self, point_cloud,min_depth, max_depth):
        ...
        """
            Founding the recntangle and angle by point cloud and depth range
            Input: 
                point_cloud: array - > point cloud.
                min_depth : float -> min depth for current region.
                max_depth : float -> max depth for current region.
            Output:
                recangle : array -> current rectangle coord..
                angle: float -> angle for the founding rectangle.
    ...
    def get_dimenssion(self, pc_rectangle, min_depth):
        """
            Input:
                 pc_rectangle: array -> point cound for founding rectangle corner, shape should be (4,2).
                 min_depth: float ->   min depth for current region.

            Output:
                w,l,h : float -> width, length, height for box.
    ...
    def classification(self,w,l,h):
        ...
        """
            Input:
                w,l,h : float -> width, length, height for box.
            Output:
                box id: str or float -> box id
                x,y  :float or int -> true width and lenght for cuurent box id
    ...
    def get_result(self):
        """               
            Found box by give point clound. 
            Output:
               {
                box_id :  float or int -> box id for founding box 
                angle: float or int -> angle for founding box 
                rectangle : array -> rectangle_array for founding box 
