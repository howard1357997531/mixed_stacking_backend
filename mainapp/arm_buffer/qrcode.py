...
class qrClass():
    ...
    #     adjusted = cv2.convertScaleAbs(gray, alpha=1.5, beta=255-factor)
    ...
    #     print(len(contours))
        ...
        return result    
    #
    def reduce_glare(self,original_image):
      

        # Convert the image to grayscale
        gray_image = cv2.cvtColor(original_image, cv2.COLOR_BGR2GRAY)

        # Apply GaussianBlur to reduce noise and glare
        ...
        for idx, cnt in enumerate(points_):
            if cv2.contourArea(cnt) > 7500:
                ...
                if len(self.image.shape) == 3:
                    ...
                    mask_angle = np.zeros_like(self.image[:,:])
