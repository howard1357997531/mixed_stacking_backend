...
class Dimension_2_3D():
    ...
    def downsampling(self, data):
        return data[::self.downsampling_factor]
    ...
    def get_dimenssion(self, point_cloud, full_depth_mask):
        ...
        vertices = self.downsampling(vertices)
        full_depth_mask = self.downsampling(full_depth_mask.flatten())
    ...
    def get_rect_angle(self,full_depth_mask):
        ...
        if len(self.contours) >1 :
            contours =  np.vstack(self.contours)
        else:
            contours = self.contours
        contours = contours + np.asarray([self.xmin, self.ymin])

        cv2.drawContours(self.image, contours, -1, (0,255,255), 3)
        cv2.imshow('image_', self.image)
